...
export default function StaffLoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    } else if (success && user?.role === 'vendor') {
      navigation.reset({ index: 0, routes: [{ name: 'VendorOrders' }] });
    } else if (success) {
      useAuthStore.getState().logout();
      setLocalError('Access denied. Admin or Vendor account required.');
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          <Text style={styles.title}>Student Affairs</Text>
          <Text style={styles.subtitle}>Admin and vendor portal</Text>
          <View style={styles.form}>
            ...
            <TouchableOpacity style={[styles.submitButton, isLoading && { opacity: 0.6 }]} onPress={handleLogin} disabled={isLoading}>
              {isLoading ? <ActivityIndicator color={Colors.white} /> : <Text style={styles.submitText}>Login</Text>}
            </TouchableOpacity>
          </View>
        </ScrollView>
